...
class ClusterAwareImportanceScorer:
    ...
    def calculate_scores(self, dense_cluster_model, client_models_in_cluster, client_gradients_in_cluster):
        ...
        Args:
            dense_cluster_model: The trained dense model for the cluster.
            client_models_in_cluster: List of client personal models.
            client_gradients_in_cluster: List of client gradient dicts.
        ...
        for layer in dense_cluster_model.layers:
            ...
            # 3. Gradient consistency scores (Normalized Sign Agreement)
            client_grads_list = []
            ...
            if client_grads_list:
                # Stack grads: shape (N_clients, ...weight_shape)
                client_grads_array = np.array(client_grads_list)
                # Get the sign of each gradient: +1, -1, or 0
                client_signs = np.sign(client_grads_array)
                # Calculate the average sign
                average_sign = np.mean(client_signs, axis=0)
                # Take abs value to get score from 0 (no agreement) to 1 (full agreement)
                consistency_scores = np.abs(average_sign)
            else:
                # Placeholder if no gradients are available
...
class AdaptivePruningScheduler:
    ...
    def get_sparsity_for_cluster(self, client_entropy_scores, num_clients_in_cluster):
        ...
        Args:
            client_entropy_scores: List of label entropy scores from each client.
            num_clients_in_cluster: number of clients in cluster
            
        Returns:
            float: target sparsity (0-1)
        """
        # 1. Calculate average label entropy (data complexity)
        if not client_entropy_scores:
            avg_entropy = 0
        else:
            avg_entropy = np.mean(client_entropy_scores)
...
class CAAFPServer:
    ...
    def cluster_clients(self, client_updates):
        ...
        # FIXED: sklearn clustering compatibility (metric vs affinity)
        try:
    ...
    def apply_cluster_aware_pruning(self, cluster_id, client_models_in_cluster,
                                    client_entropy_scores, client_gradients_in_cluster):
        """
        ...
        target_sparsity = self.pruning_scheduler.get_sparsity_for_cluster(
            client_entropy_scores, num_clients
        )
...
class CAAFPClient:
    ...
    def train_with_regularization(self, cluster_weights, epochs=5):
        ...
        FIXED: epochs=1 in cluster_model.fit to avoid double-counting.
        """
        ...
        for epoch in range(epochs):
            ...
            # FIXED: epochs=1 (was epochs=epochs causing double-counting)
    ...
    def compute_gradients(self, model_weights):
        """Compute and return gradients for consistency scoring."""
        # Set the cluster model to the provided weights
        self.cluster_model.set_weights(model_weights)
        
        gradients = {}
        
        # Take one batch to compute gradients
        for x, y in self.train_dataset.take(1):
            ...
            # Map trainable weights to layers
    ...
    def get_local_label_entropy(self):
        """
        PRIVACY FIX: Compute label entropy locally without sharing raw labels.
        """
        all_labels = []
        ...
        local_entropy = entropy(probabilities)
...
def run_caafp(num_clients=30, num_rounds=100, initial_rounds=10,
    ...
    for cluster_id in range(server.num_clusters):
        ...
        # Collect client models (for coherence score)
        client_models = [clients[cid].personal_model for cid in cluster_client_ids]
        
        # --- PRIVACY FIX: Collect local entropy scores, NOT raw labels ---
        print(f"  Cluster {cluster_id}: Collecting local entropy scores...")
        client_entropies = []
        for client_id in cluster_client_ids:
            client_entropies.append(clients[client_id].get_local_label_entropy())
        
        # --- GRADIENT CONSISTENCY: Collect gradients ---
        print(f"  Cluster {cluster_id}: Collecting gradients for consistency scoring...")
        client_gradients = []
        cluster_model_weights = server.cluster_models[cluster_id].get_weights()
        for client_id in cluster_client_ids:
            grads_dict = clients[client_id].compute_gradients(cluster_model_weights)
            client_gradients.append(grads_dict)
        
        # Apply cluster-aware adaptive pruning
        pruned_model = server.apply_cluster_aware_pruning(
            cluster_id,
            client_models,
            client_entropies,  # <-- Privacy-safe
            client_gradients   # <-- Implemented
        )
    ...
    for cluster_id in range(server.num_clusters):
        if cluster_results[cluster_id]:
            ...
            # Check if model exists before getting sparsity
...
if __name__ == "__main__":
    ...
    # --- Setup logging ---
    run_id = f"caafp_seed_{SEED}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"--- Starting CA-AFP Run: {run_id} with seed {SEED} ---")
    ...
    try:
        ...
        for cluster_id, model in server.final_pruned_models.items():
            save_path = f'saved_models/model_caafp_{run_id}_cluster_{cluster_id}.h5'
