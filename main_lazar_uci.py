...
class PostTrainPruningClient:
    ...
    def __init__(self, client_id, train_dataset, test_dataset):
        ...
        self.model = create_lstm_model(input_shape=(128, 9))
...
def run_lazarevich_baseline(num_clients=30, total_dense_rounds=40,
    ...
    dense_model = create_lstm_model(input_shape=(128, 9))
...
if __name__ == "__main__":
    ...
    # --- 1. PARSE COMMAND LINE ARGS FIRST ---
    parser = argparse.ArgumentParser()
    parser.add_argument('--seed', type=int, default=42, help='Random seed')
    args = parser.parse_args()
    
    SEED = args.seed  # <-- Define SEED from parsed args
    ...
    os.environ['TF_DETERMINISTIC_OPS'] = '1' # For TF reproducibility

    # --- 3. Setup logging and paths (now SEED is available) ---
    run_id = f"lazar_seed_{SEED}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"--- Starting Lazarevich et al. Run: {run_id} with seed {SEED} ---")
    ...
    try:
        ...
        results_0_shot, results_ft, final_pruned_model = run_lazarevich_baseline(
            num_clients=30,
            total_dense_rounds=40,      # Match CA-AFP (10 initial + 30 cluster)
