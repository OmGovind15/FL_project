...
class FedCHARClient:
    def __init__(self, client_id, train_dataset, test_dataset, lambda_reg=0.01):
        ...
        self.group_model = create_lstm_model(input_shape=(128, 9))
    ...
    def train_fedchar(self, global_weights, epochs=5):
        ...
        FIXED: epochs=1 in group_model.fit to avoid double-counting.
        """
        ...
        for epoch in range(epochs):
            ...
            self.group_model.fit(self.train_dataset, epochs=1, verbose=0)
...
def run_fedchar(num_clients=30, num_rounds=100, initial_rounds=10,
    ...
    """
    print("="*60)
    print("FedCHAR: Federated Learning with Clustered HAR")
    print("="*60)
    ...
    print(f"\n{'='*60}")
    print(f"PHASE 1: Initial Training ({initial_rounds} rounds)")
    print(f"{'='*60}")
    ...
    print(f"\n{'='*60}")
    print(f"PHASE 3: Personalized Training ({num_rounds - initial_rounds} rounds)")
    print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    # --- Setup logging and paths ---
    run_id = f"fedchar_seed_{SEED}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    print(f"--- Starting FedCHAR Run: {run_id} with seed {SEED} ---")
    ...
    try:
        ...
        # FedCHAR saves the dense cluster models (no pruning)
