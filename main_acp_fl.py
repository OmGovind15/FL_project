...
def load_data():
    ...
    print(f"Data loaded for {len(all_client_ids)} clients.")
    return client_train_loaders, client_test_loaders, all_client_ids

# ... (HARModel class, get_low_magnitude_params, client_update, calculate_alpha, flatten_model_params remain the same) ...
...
def get_low_magnitude_params(model, proportion):
    params = torch.cat([p.view(-1) for name, p in model.named_parameters() if 'weight' in name])
    k = int(proportion * params.numel())
    if k == 0: return torch.tensor(0.)
    threshold = torch.kthvalue(torch.abs(params), k).values
    return params[torch.abs(params) < threshold]

def client_update(client_loader, model, sparsity):
    local_model = copy.deepcopy(model); local_model.train()
    optimizer = optim.Adam(local_model.parameters(), lr=HP["learning_rate"]); criterion = nn.CrossEntropyLoss()
    mask = {}
    with torch.no_grad():
        for name, param in local_model.named_parameters():
            if 'weight' in name:
                ...
                if k > 0:
                    threshold = torch.kthvalue(torch.abs(param.view(-1)), k).values
                    mask[name] = (torch.abs(param) >= threshold).float()
                else:
    ...
    for _ in range(HP["local_epochs"]):
        for data, target in client_loader:
            ...
            low_mag_params = get_low_magnitude_params(local_model, proportion=HP["BAE_PROPORTION"])
            bae_loss = HP["BAE_LAMBDA"] * torch.norm(low_mag_params, p=2)
            total_loss = loss + bae_loss; total_loss.backward()
            with torch.no_grad():
                for name, param in local_model.named_parameters():
                    if name in mask: param.grad *= mask[name]
            optimizer.step()
            with torch.no_grad():
                for name, param in local_model.named_parameters():
                    if name in mask: param.data *= mask[name]
    return local_model
...
# ... (AsyncServer class remains mostly the same) ...
class AsyncServer:
    ...
    def process_finished_clients(self):
        ...
        for client_id in finished_ids:
            ...
            local_model = client_update(self.client_train_loaders[client_id], model_to_train, sparsity=HP["SPARSITY"])
    ...
    def train(self):
        ...
        while self.updates_processed < HP["max_updates"]:
            ...
            elif len(self.active_clients) > 0: # If no updates, but clients are working, advance time
                 self.updates_processed +=1
            elif len(self.free_clients) == len(self.all_client_ids): # All clients are free, nothing to do
                 break
...
if __name__ == "__main__":
    ...
    print("\n--- Starting Final ACP-FL Training ---")
    server.train()
    print("\n--- ACP-FL Training Finished ---")
